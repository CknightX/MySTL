...
namespace CK_STL
{
	...
	protected:
		...
		void deallocate_node(T* p){ data_allocator::deallocate(p, deque_buf_size(sizeof(T))); }
	...
	{
		size_type num_nodes = num_elements / buffer_size() + 1; //所需要节点数,如果整除则多分配一个
		map_size = max(initial_map_size(), num_nodes + 2);//一个map所管理的节点，最少8个，最多为所需节点+2（为扩充备用）
	...
	template<class T, class Alloc = alloc, size_t BufSize = 0 >
	void deque<T, Alloc, BufSize>::reserve_map_at_front(size_type nodes_to_add = 1) //尾插无备用空间时对map的操作
	{
	...
	void deque<T, Alloc, BufSize>::reallocate_map(size_type nodes_to_add, bool add_at_front)
	{
		...
		if (map_size > 2 * num_nodes) //使用的空间实则不足原map的一半
		{
